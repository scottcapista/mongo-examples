...
# 4. Web UI -> MCP -> Bedrock (may fail on IAM)
...
if echo "$STREAM" | grep -qE 'AccessDeniedException|Grove API [45]'; then
  ...
  if echo "$STREAM" | python3 -c "import sys,json; d=json.load(sys.stdin); t=(d.get('content') or {}).get('text',''); sys.exit(0 if t and not t.startswith('Error:') else 1)" 2>/dev/null; then
    pass "Full chat path (Bedrock + MCP)"
  else
...
# 5. Direct memory intake/recall (no Bedrock)
...
echo ""
if [[ "${BEDROCK_FAIL:-0}" -eq 1 ]]; then
  echo "E2E partial: UI + MCP + memory OK; Bedrock IAM still required for chat."
  exit 2
fi
