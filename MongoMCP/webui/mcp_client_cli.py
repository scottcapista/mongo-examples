...
def run_cli(processor: Optional[CachedQueryProcessor] = None) -> None:
    ...
    proc = processor or CachedQueryProcessor(settings, handle_message)
    ...
    try:
        answer, _, _history = proc.query_with_mcp_tools("what collections are available?")
        print(f"first Answer: {answer}")
        answer, _, _history = proc.query_with_mcp_tools("what claim status for nicole weber?")
        ...
        while True:
            ...
            if not user_input:
                ...
                answer, _, _history = proc.query_with_mcp_tools(user_input)
                answer = None
