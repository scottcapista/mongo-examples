...
export default function SessionTimelineChart({ points, bucketUnit }) {
  const chart = useMemo(() => {
    ...
    const width = 720
    const height = 280
    const pad = { top: 16, right: 48, bottom: 56, left: 56 }
    ...
    const lines = SERIES.filter((s) => s.axis === 'tokens').map((s) => ({
      ...s,
      path: data
        .map((p, i) => `${i === 0 ? 'M' : 'L'} ${xAt(i)} ${yToken(p[s.key] || 0)}`)
        .join(' '),
    }))
    ...
    return { width, height, pad, innerH, tokenMax, eventMax, lines, eventBars, xLabels }
  }, [points])
  ...
  return (
    <div className="session-timeline">
      ...
      >
        ...
        {[0, 0.25, 0.5, 0.75, 1].map((t) => {
          ...
          return (
            <g key={t}>
              ...
              <text x={chart.width - chart.pad.right + 8} y={y + 4} textAnchor="start" className="session-timeline__axis-label session-timeline__axis-label--right">
        ...
        <text x={chart.width - 12} y={chart.pad.top + 8} textAnchor="end" className="session-timeline__axis-title">Events</text>
      </svg>
    </div>
  )
}
